...
def eval_fn(args, model, data_loader, mode="valid"):
    ...
    for batch in data_loader:
        ...
        x = lg.edge_timestamp.unsqueeze(1).float()
        out = model(x, H, lg.edge_index)
...
def train(args):
    ...
    args.tsae_in_size = args.timestamp_size
    args.tsae_hidden_size = args.hidden_size
    args.tsae_out_size = args.hidden_size
    ...
    with mlflow.start_run():
        ...
        train_loader = NeighborLoader(data, num_neighbors=[10, 5], batch_size=args.batch_size, directed=False, shuffle=True,
                                      input_nodes=data.train_nodes)
        valid_loader = NeighborLoader(data, num_neighbors=[-1], batch_size=args.batch_size // 4, directed=False,
                                      input_nodes=data.valid_nodes)
        test_loader = NeighborLoader(data, num_neighbors=[-1], batch_size=args.batch_size // 4, directed=False,
                                     input_nodes=data.test_nodes)

        log_path = os.path.join("../log", args.dataset.lower(), args.expr_name)
        if not os.path.exists(log_path):
            os.makedirs(log_path)
        writer = SummaryWriter(log_dir=log_path)
        model = EdgeGCN(args).to(args.device)
