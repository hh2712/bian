...
from model_zoo.NodeEdgeAggregator import NodeEdgeAggregatorV4 as Model
...
def eval_fn(args, model, data_loader, mode="valid"):
    ...
    for batch in data_loader:
        ...
        ea = lg.x.float()
        out = model(batch.x, et, ea, H, batch.edge_index, lg.edge_index)
...
def train(args):
    ...
    data = read_dataset(args.dataset, args.dataset_dir, coalesce=True, attr=args.attr, reduce=args.reduce)
    args.num_features = data.x.size(1)
    args.edge_attr_size = data.edge_attr.size(1)
    ...
    handler = logging.FileHandler(osp.join(log_path, 'log.txt'))
    ...
    with mlflow.start_run():
        ...
        valid_loader = NeighborLoader(data, num_neighbors=[10, 5], batch_size=512, directed=False, input_nodes=data.valid_nodes)
        test_loader = NeighborLoader(data, num_neighbors=[10, 5], batch_size=512, directed=False, input_nodes=data.test_nodes)
        ...
        for ep in range(args.epochs):
            ...
            for i, batch in enumerate(train_loader):
                ...
                et = lg.edge_timestamp.unsqueeze(1).float()
                ea = lg.x.float()
                # logger.info("{}s for preprocessing".format(round(time() - start_time, 2)))

                start_time = time()
                out = model(batch.x, et, ea, H, batch.edge_index, lg.edge_index)
                # logger.info("{}s for model computation".format(round(time() - start_time, 2)))
            ...
            if (ep - max_ep >= 10):
                break
            logger.info("take {}s for epoch {}".format(round(time() - epoch_start_time, 2), ep + 1))
        mlflow.pytorch.log_model(model, "model")
        ...
        logger.info(test_acc, test_auroc, test_prec, test_recall)
